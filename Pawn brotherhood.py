...
def safe_pawns(pawns):
    count = 0
    for i in pawns:
        left_letter = ascii_lowercase[ascii_lowercase.index(i[:1:])-1]
        right_letter = ascii_lowercase[ascii_lowercase.index(i[:1:])+1]
        if (left_letter + str(int(i[1::])-1) in pawns or right_letter + 
                str(int(i[1::])-1) in pawns):
            if ((not ascii_lowercase.index(right_letter) > 7) or 
                    (not int(i[1::])-1 < 1)):
                count += 1
    print((str(int(i[1::])-1)) + left_letter)
    print((str(int(i[1::])-1)) + right_letter)
    print(count)
    return count
...
least a basic understanding of its rules. 
It has various units with a wide range of movement patterns allowing for a 
huge number of possible different game positions 
...
pair — a letter and a number (ex, "a1", "h8", "d6"). 
For this mission we only need to concern ourselves with pawns. 
A pawn may capture an opponent's piece on a square diagonally in front of it 
on an adjacent file, by moving to that square. 
For white pawns the front squares are squares with greater row number than the 
square they currently occupy.

A pawn is generally a weak unit, but we have 8 of them which we can use to 
build a pawn defense wall. With this strategy, 
one pawn defends the others. A pawn is safe if another pawn can capture 
a unit on that square. 
